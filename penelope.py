...
class MainMenu(cmd.Cmd):
    ...
    @staticmethod
    def options(text):
        _options = (option for option in dir(options)
            ...
            if ID == 'all':
                ...
                try:
                    if session_count > 1:
                        ...
                        answer = input(
                            f"\r{paint(f'[?] Kill all ({session_count}) sessions? (y/N): ','yellow')}")
    ...
    def do_exit(self, line):
        ...
        try:
            readline.set_auto_history(False)
            answer = input(
                f"\r{paint('[?] Are you sure you want to exit? (y/N): ','yellow')}")
            readline.set_auto_history(True)
        except EOFError:
            # print('\r')
            ...
            for readable in readables:
                ...
                # The listeners
                    ...
                    threading.Thread(target=Session, args=(
                        socket, *endpoint, readable)).start()
...
class Listener:
    ...
    def __init__(self, host=None, port=None):
        ...
        try:
            ...
            logger.error(
                f"Cannot bind to port {self.port}: Insufficient privileges")
    ...
    def hints(self):
        ...
        if self.host == '0.0.0.0':
            for ip in Interfaces().list.values():
                output.extend(
                    [preset.format(paint(ip, 'cyan', 'DIM'), self.port) for preset in presets])
                output.append('')
        else:
            output.extend([preset.format(self.host, self.port)
...
class Session:
    ...
    def __init__(self, _socket, target, port, listener=None):
        ...
        if self:
            ...
            logger.info(
                f"Got {self.source} shell from {OSes[self.OS]} {paint(self.name,'white','RED')}{paint('','green',reset=False)} 💀 - Assigned SessionID {paint('<'+str(self.id)+'>','yellow')}")
            ...
            logger.error(
                f"Invalid shell from {paint(self.name,'RED','white')}{paint('','red',reset=False)} 🙄\r")
            self.kill()
    ...
    def determine(self):
        ...
        response = self.exec(cmd, expect=(
            cmd.encode(), outcome, b"Windows PowerShell"))
        r = re.match(
            rf"(Microsoft Windows \[Version (.*)\].*){re.escape(cmd)}".encode(), response, re.DOTALL)
        _r = re.match(rf"(.*){outcome.decode()}".encode(), response, re.DOTALL)
        if r:  # cmd
            self.OS = 'Windows'
            self.type = 'Basic'
            ...
            r2 = re.match(rf"(.*){re.escape(cmd)}".encode(), _r[1], re.DOTALL)
            if r2:
    ...
    def exec(self, cmd=None, raw=True, timeout=None, expect=[], clear=True):
        try:
            ...
            with io.BytesIO() as control_buffer:
                while True:
                    ...
                    readables, _, _ = select.select(
                        [self.socket, self.control], [], [], timeout)
                    ...
                    if self.control in readables:
                        for command in self.control.read():
                            logger.debug(
                                f"Control pipe ID {self.id}: {command}")
                            if command == b'stop':
                                break
                    ...
                    if self.socket in readables:
                            ...
                            for byte in data:
                                ...
                                if not echoable:
                                    ...
                                    if not timeout == self.latency:
                                        ...
                                        logger.debug(
                                            f"{paint('Switched to Latency (2)','yellow')}")
                        ...
                        if not timeout == self.latency:
                            timeout = self.latency
                            logger.debug(
                                f"{paint('Switched to Latency (1)','yellow')}")
                    else:
                        if timeout != self.latency:
                            ...
                            logger.debug(
                                paint("Latency expired", 'RED', 'white'))
                            if not raw:
                                result = re.search(
                                ...
                                if result:
                                    ...
                                    logger.debug(
                                        paint('Got all data!', 'green'))
                                    output = result[1]
                                    break
                                else:
                                    logger.debug(
                                        paint('Did not get all data. Receive again...', 'yellow'))
                                ...
                                if expect:
                                    for token in expect:
                                        if token in control_buffer.getvalue():
                                            logger.debug(
                                                paint(f"Token {token} found in data", 'yellow'))
                                            break
                                    else:
                                        logger.debug(
                                            paint('No token found in data. Receive again...', 'yellow'))
                                        timeout = initial_timeout
                                        continue
                                ...
                                logger.debug(
                                    paint('Maybe got all data.', 'yellow'))
                                output = control_buffer.getvalue()
                                break
            ...
            logger.debug(
                paint("Connection terminated abnormally", 'RED', 'white'))
            self.lock.release()
            self.exit()
            return b''
    ...
    def upgrade(self):
        ...
        if self.OS == "Unix":
            ...
            if token in response.decode():
                ...
                logger.error(
                    "Cannot obtain PTY shell - python does not exist on target. Attempting to obtain Advanced shell...")
                ...
                if not token in response.decode() and not response.startswith(b'sh: '):
                    ...
                    logger.error(
                        "bash does not exist on target. Falling back to basic shell support")
    ...
    def attach(self):
        ...
        if not options.no_log:
            logger.info(
                f"{paint('Logging to ','green')}{paint(self.logpath,'yellow','DIM')} 📜")
    ...
    def download(self, remote_item_path):
        if self.OS == 'Unix':
            try:
                ...
                if not items:
                    logger.warning(
                        "The item does not exist or access is denied")
                    return []

                top_level_items = {
                    re.match(f'[^{os.path.sep}]*', item)[0] for item in items}
                ...
                for item in top_level_items:
                    ...
                    if local_item_path.exists():
                        ...
                        logger.debug(
                            f"{local_item_path} exists. Renamed to {new_path}")
                ...
                for glob in remote_globs:
                    specified.update(
                        set(local_download_folder.glob(glob.lstrip('/'))))
                specified = specified.intersection(downloaded)

                for item in specified:
                    logger.info(
                        f"Successful download! {paint(pathlink(item),'DIM','yellow')}")
    ...
    def upload(self, local_item_path):
        if self.OS == 'Unix':
            ...
            if local_item_path.startswith('http'):
                items = [
                    urlopen(local_item_path, timeout=options.SHORT_TIMEOUT).read()]

            elif local_item_path.startswith(os.path.sep):
                items = list(Path(os.path.sep).glob(
                    local_item_path.lstrip(os.path.sep)))
            else:
            ...
            if not response:
                for item in altnames:
                    logger.info(
                        f"Successful upload! {paint('('+str(item)+')','DIM','yellow')}")
            else:
        ...
        if self.OS == "Unix":
            if self.listener:
                ...
                if response.startswith(b'sh: '):
                    logger.error(
                        "Bash does not exist on target. Cannot spawn reverse shell...")
            else:
...
class Interfaces:
    ...
    def __str__(self):
        ...
        for name, ip in self.list.items():
            _list.append(
                f"{paint(name,'cyan')}{paint(':','magenta')}{paint(ip,'yellow')}")
        return '\n'.join(_list)
...
class Color:
    ...
    codes = {'RESET': 0, 'BRIGHT': 1, 'DIM': 2,
             'UNDERLINE': 4, 'BLINK': 5, 'NORMAL': 22}

    def escape(codes): return f"\x1b[{codes}m"

    def __init__(self):
        __class__.codes.update(
            {color: code for code, color in enumerate(__class__.colors, 30)})
        __class__.codes.update(
            {color.upper(): code for code, color in enumerate(__class__.colors, 40)})

    def __call__(self, text, *colors, reset=True):
        code_sequence = ';'.join(
            [str(__class__.codes[color]) for color in colors])
        prefix = __class__.escape(code_sequence) if code_sequence else ''
        suffix = __class__.escape(
            __class__.codes['RESET']) if prefix and reset else ''
...
class Options:
    log_levels = {"silent": logging.WARNING,
                  "extra_silent": logging.CRITICAL, "debug": logging.DEBUG}
...
parser.add_argument(
    "PORT", nargs='?', help="Port to listen/connect to depending on -i/-c options. Default: 4444", default=4444)

method = parser.add_argument_group("Reverse or Bind shell?")
method.add_argument(
    "-i", "--address", help="IP Address or Interface to listen on. Default: 0.0.0.0", default="0.0.0.0", metavar='')
...
verbosity.add_argument(
    "-Q", "--silent", help="Show only errors and warnings", action="store_true")
verbosity.add_argument("-X", "--extra-silent",
                       help="Suppress all logging messages", action="store_true")

log = parser.add_argument_group("Logging")
log.add_argument("-L", "--no-log",
                 help="Do not create session log files", action="store_true")
log.add_argument("-T", "--no-timestamps",
                 help="Do not include timestamps on logs", action="store_true")

misc = parser.add_argument_group("Misc")
misc.add_argument("-H", "--no-history",
                  help="Disable shell history on target", action="store_true")
misc.add_argument(
    "-P", "--plain", help="Just land to the menu", action="store_true")
...
debug.add_argument("-NP", "--no-python",
                   help="Simulate python absence on target", action="store_true")
debug.add_argument("-NB", "--no-bash",
                   help="Simulate bash absence on target", action="store_true")
...
def pathlink(filepath): return (f'\x1b]8;;file://{filepath.parents[0]}\x07{filepath.parents[0]}'
                                f'/\x1b]8;;\x07\x1b]8;;file://{filepath}\x07{filepath.name}\x1b]8;;\x07')


def Open(item): return subprocess.Popen(({'Linux': 'xdg-open', 'Darwin': 'open'}
                                         [OS], item), stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)


def rand(): return ''.join(random.choice(string.ascii_letters)
                           for i in range(8))
